...
if (prevBtn && nextBtn) {
  const slideIds = ['tourPanel', 'scene', 'sidePanels'];
  ...
  const defaultDisplays = {
    tourPanel: tourPanel?.style.display ?? '',
    scene: scene?.style.display ?? '',
    sidePanels: sidePanels?.style.display ?? ''
  };
  let lastResizeKey = '';
  ...
  const getViewportMetrics = () => {
    ...
    return { width, height, offsetLeft, offsetTop, baseWidth, baseHeight, dpr, vv };
  };
  ...
  const applyPanelVisibility = () => {
    if (!tourPanel || !scene || !sidePanels) return;
    if (isMobile()) {
      tourPanel.style.display = (document.body.dataset.activeSlide === 'tourPanel') ? 'block' : 'none';
      ...
      tourPanel.style.display = defaultDisplays.tourPanel;
      scene.style.display = defaultDisplays.scene;
      sidePanels.style.display = defaultDisplays.sidePanels;
